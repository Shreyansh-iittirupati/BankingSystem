...
public class AccountService implements AccountOperations {
    ...
    private void loadAccounts() {
        ...
        try (Connection conn = DatabaseConnector.getConnection();
            ...
            DatabaseConnector.releaseConnection(); // Release after load
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error loading accounts", e);
            DatabaseConnector.releaseConnection(); // Release on error
        }
    }
    ...
    public Account createAccount(String username, String accountType, BigDecimal initialBalance) {
        ...
        try (Connection conn = DatabaseConnector.getConnection();
            ...
            if (rowsAffected > 0) {
                ...
                DatabaseConnector.releaseConnection(); // Release after success
                return account;
            }
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error creating account", e);
        }
        DatabaseConnector.releaseConnection(); // Release on error or failure
        return null;
    }
        ...
        try (Connection conn = DatabaseConnector.getConnection();
            ...
            if (rowsAffected > 0) {
                ...
                if (!"ACTIVE".equals(newStatus)) {
                    accounts.remove(account); // Remove from observable list if not active
                }
        ...
        try (Connection conn = DatabaseConnector.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    if (!accounts.contains(account) && account.isActive()) {
                        accounts.add(account);
                    }
                }
            }
    ...
    public BigDecimal getAccountBalance(String accountId) {
        Account account = getAccount(accountId);
        DatabaseConnector.releaseConnection(); // Release after operation
            ...
            if (rowsAffected > 0) {
                ...
                accounts.stream()
                        ...
                        .ifPresent(a -> {
                            ...
                            if (!active) {
                                accounts.remove(a); // Remove from observable list if suspended
                            }
                        });
