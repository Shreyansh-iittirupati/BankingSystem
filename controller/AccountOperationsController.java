...
public class AccountOperationsController implements Initializable {
    ...
    private void populateAccountList() {
        if (currentCustomer != null && accountService != null) {
            ...
            if (userAccounts != null && !userAccounts.isEmpty()) {
                ...
                if (accountComboBox != null) accountComboBox.setItems(userAccountItems);
                if (updateAccountComboBox != null) updateAccountComboBox.setItems(userAccountItems);
                logger.info("Populated ComboBoxes with " + userAccountItems.size() + " accounts for " + username);
                if (!userAccountItems.isEmpty()) {
                    if (accountComboBox != null) accountComboBox.getSelectionModel().select(0);
                    if (updateAccountComboBox != null) updateAccountComboBox.getSelectionModel().select(0);
                    logger.info("Auto-selected account: " + userAccountItems.get(0));
                }
            } else {
    ...
    private void handleViewAccountDetails() {
        if (currentCustomer != null && accountService != null) {
            ...
            if (account != null) {
                ...
                Platform.runLater(() -> {
                    if (statusLabel != null) statusLabel.setText(details);
                    logger.info("Displayed details for account: " + accountId + " in statusLabel");
                    if (statusLabel != null) statusLabel.requestLayout();
                });
            } else {
                Platform.runLater(() -> {
                    if (statusLabel != null) statusLabel.setText("Account not found for ID: " + accountId);
                    AlertHelper.showInformationAlert("Info", "Account Not Found", "No details available for the selected account.");
                });
            }
        } else {
            Platform.runLater(() -> {
    ...
    private void handleUpdateAccount() {
        if (currentCustomer != null && accountService != null) {
            ...
            // Create a dialog for account type selection
            ...
            // Update account type and status separately
            ...
            if (typeUpdated && statusUpdated) {
                ...
                populateAccountList(); // Refresh the account list to reflect changes
            } else {
                Platform.runLater(() -> {
                    if (statusLabel != null) {
    ...
    private void handleBackToMain() {
        if (app != null && app.getSceneManager() != null) {
            app.getSceneManager().switchToDashboardScene(); // Changed to switch to dashboard
            Platform.runLater(() -> {
